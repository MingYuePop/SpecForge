...
**有 UI 变化的任务**：生成手动验收清单，交由用户在实际页面上验证。TDD 的单元测试/组件测试验证的是代码逻辑正确性，不能替代真实浏览器中的端到端体验——组件渲染正常不代表页面整体表现正确，这一步由人来判断最准确。

生成清单的流程：
1. 读取 `assets/manual-verification-template.md`
2. 根据当前任务的验证标准和对应 AC，生成具体的验证项
3. 验证项按用户操作顺序排列，每条包含：具体操作步骤、预期可观测结果、AC 追溯
4. 保存到 `docs/验收清单/{功能名}_Task-{XX}_验收清单.md`
5. 告知用户验收清单已生成，请按清单验证并反馈结果

**纯后端/工具函数任务**：TDD 阶段的单元测试已覆盖核心逻辑，验收自然通过，无需额外操作。

**涉及数据库变更的任务**：用 Supabase MCP 查询数据库，验证数据状态是否符合预期。可与上面两种方式组合。

**怎么判断**：任务涉及用户能看到的变化（页面、组件、样式、交互）→ 有 UI 变化，需要生成手动验收清单。

**用户反馈不通过时**：根据用户描述的实际行为，进入修复循环：分析原因 → 修复代码 → 重跑 TDD 测试（防回归）→ 更新验收清单中对应验证项的状态 → 请用户重新验证不通过的项。**最多重试 3 次**，仍然失败就向用户报告具体原因和已尝试的修复。

**用户提出期望调整时**：验证项通过（功能行为符合 AC），但用户对效果不满意——如动画节奏、间距、交互方式等。期望调整**不阻塞任务完成**，不在验收环节当场修改。所有期望调整统一记录在验收清单中，作为后续 feature-evolution 的输入。任务仍标记完成，继续推进。
...
按任务顺序逐个执行。每个任务都必须走完上面的**两个阶段**（TDD 循环 + 验收确认）才能标记完成。不要 TDD 通过就直接进入收尾——先判断这个任务需要什么类型的验收。有 UI 变化的任务，生成手动验收清单后**停下来等待用户反馈**，不继续执行后续任务。用户确认通过后标记完成并继续；用户反馈不通过则进入修复循环。
...
4. 检查本阶段是否有未完成验收的手动清单——如果有，提醒用户完成验收后再正式关闭阶段
...
- 有 UI 变化的任务，必须生成手动验收清单并获得用户确认后才能标记完成，不得跳过手动验收环节
- 期望调整（通过但效果不满意）不在验收环节当场处理，统一记录后走 feature-evolution 流程